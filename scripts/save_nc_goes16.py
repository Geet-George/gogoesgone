...
def main():
    ...
    if os.path.isfile(mapper_path+mapper_file) == False:
        print("No mapper with the name:", mapper_path+mapper_file)
        return    
    ...
    m = fsspec.get_mapper(f"reference::{mapper_path}{mapper_file}",
    ...
    subset_to_save = subset[["CMI", "valid_pixel_count"]] 

    # Save to daily netcdf files
    subset_to_save.to_netcdf(save_path+filename)
